...
public sealed class ProblemSolver(HttpClient client)
{
    public async Task<SolvedProblem> SolveAsync(string title,string body,string provider,string model,string? apiKey=null,IReadOnlyList<VisualPage>? images=null,CancellationToken token=default)
    {
        ...
        if(ReactionMassCheck.Solve(body) is { } verified)
            return new(verified.Answer,string.Join("\n",verified.Steps),verified.Steps,"코드 독립 검산");
...
문제를 실제로 푸는 데 필요한 판단과 계산 전환을 기준으로 풀이를 자연스러운 단계로 나눈다. 단순한 문제는 짧게, 복합 문제는 더 세분화하며 1~12개의 steps를 순서대로 작성한다. 임의로 3단계에 맞추거나 실제 논리 단계를 합치거나 늘리지 않는다.
답을 확정할 수 없으면 추측하지 말고 status="unsupported"와 이유를 message에 쓴다.
완성할 수 있으면 status="ready", message="", answer, explanation, steps(문자열 1~12개)를 JSON 객체 하나로 출력한다.
""";
    ...
    public static SolvedProblem Parse(byte[] bytes,string method)
    {
        try{
            ...
            var steps=root.GetProperty("steps").EnumerateArray().Select(x=>LocalVisionReader.NormalizeMath(x.GetString()??"")).ToArray();
            ...
            return new(answer,explanation,steps,method+" 풀이 생성");
