...
var app=builder.Build();app.Urls.Add("http://127.0.0.1:18280");
...
app.Use(async(context,next)=>{
    ...
    context.Response.Headers["Content-Security-Policy"]="default-src 'self'; img-src 'self' blob: data:; style-src 'self'; script-src 'self'; connect-src 'self'; frame-ancestors 'none'; base-uri 'self'; form-action 'self'";
...
app.MapGet("/api/status",async(CancellationToken token)=>{
    try{var model=await generator.ProbeAsync(LocalGemmaGenerator.DefaultEndpoint,token,requireVision:true);return Results.Ok(new{ready=true,vision=true,model=model.Name,deepseekConfigured=deepseekKey.Length>0,deepseekModel=DeepSeekVisualGenerator.DisplayName,context=model.ContextSize,version="0.6.0-web",publicTest=false});}
...
app.MapPost("/api/import",async(HttpRequest request,CancellationToken requestToken)=>{
    ...
    var form=await request.ReadFormAsync(token);var readingProvider=form["provider"].ToString();var materialKind=form["materialKind"].ToString();
        ...
        if(first.Draft.Source!.IsAttachment){
            ...
            if(problemOnly){
                ...
                if(cloudReading){var read=await ReadDeepSeekMaterial();material=read.Material;if(read.Retried)material=material with{Uncertainties=material.Uncertainties.Concat(["첫 이미지 분석 응답의 형식을 읽지 못해 서버가 자동으로 한 번 더 분석했습니다."]).Distinct().ToArray()};materialReaderName=DeepSeekVisualGenerator.DisplayName;}
                    ...
                    var model=await generator.ProbeAsync(LocalGemmaGenerator.DefaultEndpoint,token,true);
                    try{material=await reader.ReadMaterialAsync(pages[0],token,LocalGemmaGenerator.DefaultEndpoint,model.Id,views);materialReaderName="로컬 Gemma 4 12B";}
...
app.MapPost("/api/solution",async(SolutionInput input,CancellationToken token)=>{
    ...
    var model=input.Provider=="deepseek"?DeepSeekVisualGenerator.Model:(await generator.ProbeAsync(LocalGemmaGenerator.DefaultEndpoint,token,true)).Id;
    var solved=await problemSolver.SolveAsync(title,body,input.Provider,model,deepseekKey,images,token);
    return Results.Ok(new{body,solved.Answer,solved.Explanation,solved.Steps,solved.Method});
});
    ...
    _=Task.Run(async()=>{
        try{
            async Task GenerateOne(ProviderJob output,ProblemDraft currentDraft){
                ...
                catch(Exception e)when(e is ArgumentException or InvalidDataException or InvalidOperationException or UnsupportedProblemException){output.State="failed";output.Error=e.Message;app.Logger.LogWarning("Generation validation failed: job={JobId}, provider={Provider}, formatStage={FormatStage}, exceptionType={ExceptionType}, innerExceptionType={InnerExceptionType}",id,output.Provider,e.Data["DeepSeekFormatStage"]??"validation",e.GetType().Name,e.InnerException?.GetType().Name??"none");}
...
record GenerationInput(string? Title,string? Body,string? Answer,string? Explanation,string? SourceId=null,bool RequiresImage=false,bool FromSolution=false,string Provider="gemma",string? RequestId=null,bool UseSolutionLogic=false,string[]? LogicSteps=null,bool StageSeries=false);
